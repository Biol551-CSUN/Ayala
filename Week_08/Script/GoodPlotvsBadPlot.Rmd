...
ggplot(data = cancer, 
  ...
  labs(title='the PloT whti DaTa?!',
       ...
       x='Breast',y='Count') +
  theme_void() +
  theme(plot.title = element_text(size = 55, hjust = 0.5, color = "chartreuse4"),
        plot.title.position = "plot", # title adjustments
        plot.subtitle = element_text(size =66,hjust = 0.5, lineheight = 1.1, color = "dodgerblue"),
        axis.title = element_text(size = 68, color = "darkgoldenrod4", hjust = 0.5), # title adjustments
        axis.text = element_text(size = 47,color = "tomato1",hjust = 0.5), # text adjustments
        panel.grid = element_line(linewidth = 0.35, linetype = "dotted", color = "gold4"), # background gridlines
        plot.margin = unit(c(1.0, 1.0, 1.0, 1.0), "cm"), # margin adjustments
        plot.background = element_rect(color = "coral1", fill="black"))
### save plot ###
...
ggplot(data=data,aes(x= `deg-malig`, # degree of malignancy
  ...
  theme(axis.title = element_text(size = 30, color = "darkslateblue"),
